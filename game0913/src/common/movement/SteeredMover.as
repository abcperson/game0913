package common.movement
{
	...
	public class SteeredMover extends Mover
	{
		private var _maxForce:Number = 1;
		private var _steeringForce:Vector2D;
		private var _arrivalThreshold:Number = 100;
		private var _wanderAngle:Number = 0;
		private var _wanderDistance:Number = 500;
		private var _wanderRadius:Number = 500;
		...
		public function arrive($x:Number, $y:Number, $z:Number = 0):void {
			_tmpVector2D.x = $x;
			_tmpVector2D.y = $y;
			_tmpVector2D.z = $z;
			arriveImpl(_tmpVector2D);
		}
		
		private function arriveImpl(target:Vector2D):void
		{
			var desiredVelocity:Vector2D = target.subtract(_position);
			desiredVelocity.normalize();
		...
		public function pursue(target:Mover):void
		{
		...
		public function evade(target:Mover):void
		{
			var lookAheadTime:Number = position.dist(target.position) / _maxSpeed;
		...
		public function followPath(path:Array, loop:Boolean = false):void
		{
			...
			{
				arrive(wayPoint.x, wayPoint.y, wayPoint.z);
			}
			else
			{
				seek(wayPoint);
		...
		public function flock(vehicles:Array):void
		{
			...
			{
				var vehicle:Mover = vehicles[i] as Mover;
		...
		public function inSight(vehicle:Mover):Boolean		
		{
		...
		public function tooClose(vehicle:Mover):Boolean
		{
